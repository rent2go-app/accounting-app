...
// Broad on purpose — catches informal extension/pay-later intent so morning reminders acknowledge it.
// Over-flagging is harmless (the drafter reads the full text and decides); missing a request is not.
const GRACE_RX = /\b(grace|extension|extend|more time|another day|one more day|couple (?:of )?days|few days|by (?:tomorrow|tonight|monday|tuesday|wednesday|thursday|friday|saturday|sunday|the\s+)?morning|by (?:end of|eod)|tomorrow|tonight|in the morning|later (?:today|tonight)|throughout the (?:day|week)|end of (?:the )?(?:day|week)|eod|asap|as soon as|when i (?:get off|get paid|can|finish)|get off (?:work)?|off work|pay(?:ing)?\s+(?:it|you|that|the|tomorrow|later|tonight|monday|tuesday|wednesday|thursday|friday|soon)|catch (?:it |them |up)|caught up|clear (?:it|the|my)|can'?t\s+pay|need(?:s)?\s+(?:a bit|more)?\s*time|hold\s+(?:on|off)|bear with|bare with|give me|wait (?:until|till|for)|push (?:it|to)|next week|cash ?app|shortly|payday|paycheck|make (?:a |the |my )?payment|send(?:ing)? (?:the |a |my )?payment)\b/i;
