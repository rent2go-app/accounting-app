...
async function scanAccount(acc: any) {
  ...
  await Promise.all(active.map(async (s: any) => {
    ...
    const pdSorted = [...pdinv.filter((x: any) => !isLateFee(x)).sort((a: any, b: any) => (a.created || 0) - (b.created || 0)), ...pdinv.filter((x: any) => isLateFee(x)).sort((a: any, b: any) => (a.created || 0) - (b.created || 0))];
    const _pd = pdSorted.map((i: any) => invline(i, "❌"));
    const _cur = current_open.sort((a: any, b: any) => (a.created || 0) - (b.created || 0)).map((i: any) => invline(i, "✅"));
    const parts: string[] = [];
    if (_pd.length) parts.push("Currently outstanding:\n\n" + _pd.join("\n\n") + "\n\nOutstanding subtotal: " + m(pd_total));
    if (_cur.length) parts.push("Coming due today (not yet late):\n\n" + _cur.join("\n\n") + "\n\nComing-due subtotal: " + m(current_amt));
